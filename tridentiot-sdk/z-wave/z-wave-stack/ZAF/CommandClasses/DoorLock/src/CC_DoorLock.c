...
e_cmd_handler_return_code_t CC_DoorLock_ConfigurationSet_handler(cc_door_lock_configuration_t * pData)
{
  ...
  uint8_t inside_handles_to_set = (uint8_t)(cc_door_lock_get_supported_inside_handles() & pData->insideDoorHandleMode);
  uint8_t outside_handles_to_set = (uint8_t)(cc_door_lock_get_supported_outside_handles() & pData->outsideDoorHandleMode);
