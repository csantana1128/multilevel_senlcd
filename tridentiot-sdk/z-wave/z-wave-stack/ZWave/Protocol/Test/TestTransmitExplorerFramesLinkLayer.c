...
/**
 * @file TestTransmitExplorerFramesLinkLayer.c
 ...
 */
void test_speed_to_comm_profile_always_listen_and_transmit_explorer_frame_eu(void)
{
  /***** Expectation setup *****/
  ...
  uint8_t          cmdSetNwiFrameHeaderLength = 9; // The Cmd Set Nwi header as it is expected to be seen on the phy layer.
  uint8_t          cmdSetNwiFramePayloadLength = 12; // The Cmd Set Nwi payload as it is expected to be seen on the phy layer.
  uint8_t          cmdSetNwiFullFrame[]     = {0xCA, 0xFE, 0xBA, 0xBF, 0x01, 0x05, 0x01, 0x16, 0xFF, // General Header
                                               0x20, 0x00, 0xFA, 0x40, 0x00, 0x00, 0x00, 0x00,       // Explorer header. Version = 1, Command Type = 0 (Explorer Cmd Normal),  Session Tx Random = 0xFA, Routed, direction, stop, stop req all 0, Session TTL = 4, Repeater list empty
                                               0x01, 0x22, 0x00, 0x00,                               // Z-Wave protocol vers. = 1, Cmd set nwi (0x22), Idle = 0, Timeout Default = 0
                                               0x9B};                                                // CRC
  ...
  uint8_t          cmdSetNwi[]     = {0x01, 0x22, 0x00, 0x00};
 ...
 * This test will set up the node to support EU @ 9.6k/40k/100k and the capabilities of remote node is just 9.6k / 40k.
 */
void test_speed_to_comm_profile_null_pointer_explorer_frame_eu(void)
{
  mock_t* p_mock = NULL;
 /***** Expectation setup *****/
  ZW_ReturnCode_t radioInitReturnValueExpected = SUCCESS;
