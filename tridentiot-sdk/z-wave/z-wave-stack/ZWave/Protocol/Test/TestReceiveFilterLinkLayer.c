...
/**
 * @file TestReceiveFilterLinkLayer.c
...
#ifdef BACKWARD_COMPABILITY_TEST
  TEST_ASSERT_EQUAL(23, mTestFilter1ReceivedFrame.frameContentLength);
  TEST_ASSERT_EQUAL(0, memcmp(&mTestFilter1ReceivedFrame.frameContent[9], &nifFrame[9], mTestFilter1ReceivedFrame.frameContentLength - 9));
#else  // BACKWARD_COMPABILITY_TEST
  TEST_ASSERT_EQUAL(13, mTestFilter1ReceivedFrame.frameContentLength);
  TEST_ASSERT_EQUAL(0, memcmp(mTestFilter1ReceivedFrame.frameContent, &nifFrame[9], mTestFilter1ReceivedFrame.frameContentLength));
#endif // BACKWARD_COMPABILITY_TEST
}
...
void test_receive_homeid_filter_frames(void)
{
  ...
  mp_zpal_received_frame->frame_content_length = sizeof(infoGetFrameNotMatching);
...
#else  // BACKWARD_COMPABILITY_TEST
  TEST_ASSERT_EQUAL(2, mTestFilter1ReceivedFrame.frameContentLength);
  TEST_ASSERT_EQUAL(0, memcmp(mTestFilter1ReceivedFrame.frameContent, &infoGetFrameMatching[9], mTestFilter1ReceivedFrame.frameContentLength));
#endif // BACKWARD_COMPABILITY_TEST
}
...
/** As correct radio initialization is ensured in other tests, we just utilize the helper function in this test. */
  ...
  ZW_ReceiveFilter_t rxFilter4 = {.homeId = {{0xCA, 0xFE, 0xBA, 0xBF}},
  ...
  ZW_ReceiveFilter_t rxFilter5 = {.homeId = {{0xCA, 0xFE, 0xBA, 0xBF}},
...
#ifdef BACKWARD_COMPABILITY_TEST
  TEST_ASSERT_EQUAL(12, mTestFilter5ReceivedFrame.frameContentLength);
...
#else  // BACKWARD_COMPABILITY_TEST
  TEST_ASSERT_EQUAL(3, mTestFilter0ReceivedFrame.frameContentLength);
  TEST_ASSERT_EQUAL(0, memcmp(mTestFilter5ReceivedFrame.frameContent, &infoGetFrameMatching[9], mTestFilter5ReceivedFrame.frameContentLength));
#endif // BACKWARD_COMPABILITY_TEST
}
...
/** As correct radio initialization is ensured in other tests, we just utilize the helper function in this test. */
  ...
  mp_zpal_received_frame->frame_content_length = sizeof(explorerVersionGetMatching);
  memcpy(mp_zpal_received_frame->frame_content, explorerVersionGetMatching, mp_zpal_received_frame->frame_content_length);
    radioProfile.receive_handler_cb(&rxParameters, mp_zpal_received_frame);
...
/** As correct radio initialization is ensured in other tests, we just utilize the helper function in this test. */
  ...
  mp_zpal_received_frame->frame_content_length = sizeof(explorerNifFrame);
  memcpy(mp_zpal_received_frame->frame_content, explorerNifFrame, mp_zpal_received_frame->frame_content_length);
    radioProfile.receive_handler_cb(&rxParameters, mp_zpal_received_frame);
  ...
  mp_zpal_received_frame->frame_content_length = sizeof(explorerNifRepeatFrame);
  memcpy(mp_zpal_received_frame->frame_content, explorerNifRepeatFrame, mp_zpal_received_frame->frame_content_length);
    radioProfile.receive_handler_cb(&rxParameters, mp_zpal_received_frame);
  // Explorer auto inclusion from foreign homeid, frame should not be discarded as we have a filter.
  // but should not reach the home id callback filter.
  TEST_ASSERT_FALSE(mTestFilter1Called);
...
/** As correct radio initialization is ensured in other tests, we just utilize the helper function in this test. */
  ...
  mp_zpal_received_frame->frame_content_length = sizeof(explorerFrameForeign);
  memcpy(mp_zpal_received_frame->frame_content, explorerFrameForeign, mp_zpal_received_frame->frame_content_length);
    radioProfile.receive_handler_cb(&rxParameters, mp_zpal_received_frame);
...
{
  mock_t* p_mock = NULL;
  TEST_IGNORE_MESSAGE("Currently the payload filtering in explore frames are disabled");
  zpal_radio_rx_parameters_t rxParameters = {
    .speed = ZPAL_RADIO_SPEED_40K,
    .channel_id = 1,
    .channel_header_format = ZPAL_RADIO_HEADER_TYPE_2CH,
...
/** As correct radio initialization is ensured in other tests, we just utilize the helper function in this test. */
  ...
  mTestFilter5Called = false;
#if 0
Remoed due disabled payload filtering in explore frames
  // Home ID, source, dest, payload 1 (No match)
  actualVal = llReceiveFilterAdd(&rxFilter3);
  TEST_ASSERT_EQUAL(SUCCESS, actualVal);
...
void test_receive_add_ack_filters(void)
{
  ...
  zpal_radio_rx_parameters_t rxParameters100 = {
...
/** As correct radio initialization is ensured in other tests, we just utilize the helper function in this test. */
  ...
  mp_zpal_received_frame->frame_content_length = sizeof(ackFrame9600);
  memcpy(mp_zpal_received_frame->frame_content, ackFrame9600, mp_zpal_received_frame->frame_content_length);
    radioProfile.receive_handler_cb(&rxParameters, mp_zpal_received_frame);
  TEST_ASSERT_FALSE(mTestFilter1Called);
  ...
  mTestFilter2Called = false;
  mp_zpal_received_frame->frame_content_length = sizeof(ackFrame100k);
  memcpy(mp_zpal_received_frame->frame_content, ackFrame100k, mp_zpal_received_frame->frame_content_length);
  ...
  radioProfile.receive_handler_cb(&rxParameters100, mp_zpal_received_frame);
  TEST_ASSERT_FALSE(mTestFilter2Called);
  TEST_ASSERT_TRUE(mTestFilter1Called);
  mTestFilter1Called = false;
  ...
  mTestFilter2Called = false;
  mp_zpal_received_frame->frame_content_length = sizeof(ackFrame100kForeign);
  memcpy(mp_zpal_received_frame->frame_content, ackFrame100kForeign, mp_zpal_received_frame->frame_content_length);
...
#ifdef BACKWARD_COMPABILITY_TEST
  TEST_ASSERT_EQUAL(11, mTestFilter5ReceivedFrame.frameContentLength);
  TEST_ASSERT_EQUAL(0, memcmp(&mTestFilter5ReceivedFrame.frameContent[9], &ackFrame100k[9], mTestFilter5ReceivedFrame.frameContentLength - 9));
#else  // BACKWARD_COMPABILITY_TEST
  TEST_ASSERT_EQUAL(2, mTestFilter5ReceivedFrame.frameContentLength);
  TEST_ASSERT_EQUAL(0, memcmp(mTestFilter5ReceivedFrame.frameContent, &ackFrame100k[9], mTestFilter5ReceivedFrame.frameContentLength));
#endif // BACKWARD_COMPABILITY_TEST
}
 ...
 *
 */
void test_receive_add_multicast_filters(void)
{
  mock_t* p_mock = NULL;
  zpal_radio_rx_parameters_t rxParameters = {
    .speed = ZPAL_RADIO_SPEED_40K,
  ...
  uint8_t          cmdMulticastFrameForeign[] = {0x3E, 0x27, 0x38, 0xFA, 0x02, 0x02, 0x01, 0x29, // General Header
                                                 0x1D, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Multicast frame, Address offset + Mask bytes, Mask byte. (Node: 1, 3, 4)
                                                 0x5A, 0x01, // Payload, Device Reset Locally
                                                 0x21};      // CRC
...
/** As correct radio initialization is ensured in other tests, we just utilize the helper function in this test. */
  ...
  ZW_ReceiveFilter_t rxFilter6 = {.headerType = 0x02,                                 // Multicast
                                  .flag = 0,
                                  .frameHandler = testFilter6Callback};

  ZW_ReceiveFilter_t rxFilter0 = {.homeId = {{0xE3, 0x72, 0x83, 0xAF}},
                                  .headerType = 0x02,                                 // Multicast
                                  .flag = HOMEID_FILTER_FLAG,
                                  .frameHandler = testFilter0Callback};

  ZW_ReceiveFilter_t rxFilter1 = {.homeId = {{0xE3, 0x72, 0x83, 0xAF}},
                                  .headerType = 0x02,                                 // Multicast
                                  .flag = HOMEID_FILTER_FLAG, /* | DESTINATION_NODE_ID_FILTER_FLAG,*/
                                  .frameHandler = testFilter1Callback};
  ...
  ZW_ReceiveFilter_t rxFilter3 = {.homeId = {{0xE3, 0x72, 0x83, 0xAF}},
                                  .sourceNodeId  = 1,
                                  .payloadIndex1 = 0x00, .payloadFilterValue1 = 0x1D,
                                  .payloadIndex2 = 0x01, .payloadFilterValue2 = 0x0D,
                                  .headerType = 0x02,                                 // Multicast
  ...
  ZW_ReceiveFilter_t rxFilter4 = {.homeId = {{0xE3, 0x72, 0x83, 0xAF}},
                                  .payloadIndex1 = 0x01, .payloadFilterValue1 = 0x0D,
                                  .headerType = 0x02,                                 // Multicast
                                  .flag = HOMEID_FILTER_FLAG | PAYLOAD_INDEX_1_FILTER_FLAG,
                                  .frameHandler = testFilter4Callback};
  ...
  // Pass the foreign frame. Only global explorer filter should match.
  mp_zpal_received_frame->frame_content_length = sizeof(cmdMulticastFrameForeign);
  memcpy(mp_zpal_received_frame->frame_content, cmdMulticastFrameForeign, mp_zpal_received_frame->frame_content_length);
    radioProfile.receive_handler_cb(&rxParameters, mp_zpal_received_frame);
  TEST_ASSERT_TRUE(mTestFilter6Called);
  mTestFilter6Called = false;
