...
/**
 * @file TestZW_build_tx_header_controller.c
...
#pragma GCC diagnostic ignored "-Wmissing-braces"
  ack_test_vector_t test_vector[] = {
    {{0x00,.homeId = {0xF8, 0x75, 0xE2, 0x04} ,.sequenceNumber = 0x50 ,.frameType = HDRTYP_TRANSFERACK} ,HDRFORMATTYP_2CH, 0x50, FRAME_TYPE_ACK,.mHomeID = {0xF8, 0x75, 0xE2, 0x04}, .mNodeID = 2},
    {{0x00,.homeId = {0xF8, 0x75, 0xE2, 0x04} ,.sequenceNumber = 8 ,.frameType = HDRTYP_TRANSFERACK} ,HDRFORMATTYP_3CH, 8, FRAME_TYPE_ACK,.mHomeID = {0xF8, 0x75, 0xE2, 0x04}, .mNodeID = 1},
    {{0x00,.homeId = {0xF8, 0x75, 0xE2, 0x04} ,.sequenceNumber = 8 ,.frameType = HDRTYP_TRANSFERACK} ,HDRFORMATTYP_LR, 8, FRAME_TYPE_ACK,.mHomeID = {0xF8, 0x75, 0xE2, 0x04}, .mNodeID = 1}
  };
...
#pragma GCC diagnostic ignored "-Wmissing-braces"
  single_test_vector_t test_vector[] = {
    // 2CH test vector
    {{0x00,.sequenceNumber = 5 ,.frameType = HDRTYP_SINGLECAST},
    HDRFORMATTYP_2CH, 0x50,  (0x50|FRAME_TYPE_SINGLE),
    .mHomeID = {0xF8, 0x75, 0xE2, 0x04},
    .mNodeID = 2,  .setAck = 0},
    {{0x00,.sequenceNumber = 5 ,.frameType = HDRTYP_SINGLECAST,.acknowledge = 1},
     HDRFORMATTYP_2CH, 0x50, (0x50|FRAME_TYPE_SINGLE),
     .mHomeID = {0xF8, 0x75, 0xE2, 0x04},
     .mNodeID = 2, .setAck = 1},
    {{0x00,.sequenceNumber = 5 ,.frameType = HDRTYP_SINGLECAST, .wakeup1000ms =1},
    HDRFORMATTYP_2CH, 0x50, (0x50|FRAME_TYPE_SINGLE),
    .mHomeID = {0xF8, 0x75, 0xE2, 0x04},
    .mNodeID = 2, .setAck = 0,
    .sensorMode = APPLICATION_FREQ_LISTENING_MODE_1000ms},
    {{0x00,.sequenceNumber = 5 ,.frameType = HDRTYP_SINGLECAST, .wakeup250ms = 1},
     HDRFORMATTYP_2CH, 0x50, (0x50|FRAME_TYPE_SINGLE),
     .mHomeID = {0xF8, 0x75, 0xE2, 0x04},
     .mNodeID = 2, .setAck = 0,
     .sensorMode = APPLICATION_FREQ_LISTENING_MODE_250ms},
    {{0x00,.sequenceNumber = 5 ,.frameType = HDRTYP_SINGLECAST, .wakeup1000ms = 1, .wakeup250ms = 1},
     HDRFORMATTYP_2CH, 0x50, (0x50|FRAME_TYPE_SINGLE),
     .mHomeID = {0xF8, 0x75, 0xE2, 0x04},
     .mNodeID = 2, .setAck = 0,
     .sensorMode = (APPLICATION_FREQ_LISTENING_MODE_250ms|APPLICATION_FREQ_LISTENING_MODE_1000ms)},

    // 3CH test vector
    {{0x00,.sequenceNumber = 8, .frameType = HDRTYP_SINGLECAST},
     HDRFORMATTYP_3CH, 8, FRAME_TYPE_SINGLE,
     .mHomeID = {0xF8, 0x75, 0xE2, 0x04},
     .mNodeID = 2, .setAck = 0},
    {{0x00,.sequenceNumber = 8 ,.frameType = HDRTYP_SINGLECAST,.acknowledge = 1},
     HDRFORMATTYP_3CH, 8, FRAME_TYPE_SINGLE,
     .mHomeID = {0xF8, 0x75, 0xE2, 0x04},
     .mNodeID = 2, .setAck = 1},
    {{0x00,.sequenceNumber = 8, .frameType = HDRTYP_SINGLECAST, .wakeup1000ms = 1},
     HDRFORMATTYP_3CH, 8, FRAME_TYPE_SINGLE,
     .mHomeID = {0xF8, 0x75, 0xE2, 0x04},
     .mNodeID = 2, .setAck = 0,
     .sensorMode = APPLICATION_FREQ_LISTENING_MODE_1000ms},
    {{0x00,.sequenceNumber = 8, .frameType = HDRTYP_SINGLECAST, .wakeup250ms = 1},
     HDRFORMATTYP_3CH, 8, FRAME_TYPE_SINGLE,
     .mHomeID = {0xF8, 0x75, 0xE2, 0x04},
     .mNodeID = 2, .setAck = 0,
     .sensorMode = APPLICATION_FREQ_LISTENING_MODE_250ms},
    {{0x00,.sequenceNumber = 8, .frameType = HDRTYP_SINGLECAST, .wakeup1000ms = 1, .wakeup250ms = 1},
     HDRFORMATTYP_3CH, 8, FRAME_TYPE_SINGLE,
     .mHomeID = {0xF8, 0x75, 0xE2, 0x04},
     .mNodeID = 2, .setAck = 0,
     .sensorMode = (APPLICATION_FREQ_LISTENING_MODE_250ms|APPLICATION_FREQ_LISTENING_MODE_1000ms)},
    ...
    // LR test vector
    {{0x00,.sequenceNumber = 8 ,.frameType = HDRTYP_SINGLECAST},
     HDRFORMATTYP_LR, 8, FRAME_TYPE_SINGLE,
     .mHomeID = {0xF8, 0x75, 0xE2, 0x04},
     .mNodeID = 2, .setAck = 0},
    {{0x00,.sequenceNumber = 8 ,.frameType = HDRTYP_SINGLECAST,.acknowledge = 1},
     HDRFORMATTYP_LR, 8, FRAME_TYPE_SINGLE,
...
#pragma GCC diagnostic pop
  ...
  for (uint32_t i = 0; i < sizeof_array(test_vector); i++)
  {
    ...
    mock_call_expect(TO_STR(SetTransmitHomeID), &pMock);
    pMock->expect_arg[0].pointer = &txElm;
    mock_call_expect(TO_STR(IsNodeSensor), &pMock);
    pMock->expect_arg[0].value = test_vector[i].mNodeID;
    pMock->expect_arg[1].value = true;
    pMock->expect_arg[2].value = false;
    pMock->return_code.value = test_vector[i].sensorMode;
    ZW_HomeIDSet(test_vector[i].mHomeID);
    g_nodeID = test_vector[i].mNodeID;
    txElm.bFrameOptions = 0;
    currentSeqNo = test_vector[i].SeqNo ;
...
#pragma GCC diagnostic ignored "-Wmissing-braces"
 ...
 #pragma GCC diagnostic pop
  ...
  for (uint32_t i = 0; i < sizeof_array(test_vector); i++)
  {
    ...
    mock_call_expect(TO_STR(IsNodeSensor), &pMock);
    pMock->expect_arg[0].value = dest_nodeid;
    pMock->expect_arg[1].value = false;
    pMock->expect_arg[2].value = true;
    pMock->return_code.value = test_vector[i].destIsSensor;
    if (test_vector[i].destIsSensor)
    {
      if (test_vector[i].addExHeader)
      {
        ...
        pMock->expect_arg[2].value = false;
        pMock->return_code.value = test_vector[i].destIsSensor;
        if (test_vector[i].headerType == HDRFORMATTYP_2CH)
        {
          mock_call_expect(TO_STR(IsNodeSensor), &pMock);
          pMock->expect_arg[0].value = g_nodeID;
          pMock->expect_arg[1].value = true;
          pMock->expect_arg[2].value = false;
      ...
      else
      {
        ...
        pMock->expect_arg[0].value = txElm.frame.frameOptions.sourceNodeId;
