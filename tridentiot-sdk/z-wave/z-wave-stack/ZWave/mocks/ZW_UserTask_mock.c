...
#include <mock_control.h>  // Required by the test-framework
...
#define MOCK_FILE "ZW_UserTask.c"
...
//  uint16_t stackSizeConverted = 0;
  ...
  uint8_t priority = task->priority + TASK_PRIORITY_BACKGROUND;
  ...
  taskHandle = NULL + 1;  // Not NULL
  // Mocked:      xTaskCreateStatic(
...
** StackSize is in Words, NOT bytes
**----------------------------------------------------------------------------*/
                                           ...
                                           const SProtocolConfig_t * pProtocolConfig)
{
  ...
  mainAppTaskBuffer.taskBuffer = &AppTaskBuffer;
  mainAppTaskBuffer.stackBuffer = AppStackBuffer;
  mainAppTaskBuffer.stackBufferLength = TASK_STACK_SIZE_MAIN_USER_APP;

  // Create the task setting-structure!
  ZW_UserTask_t task;
  task.pTaskFunc = (TaskFunction_t)appTaskFunc;
  task.pTaskName = TASK_NAME_MAIN_USER_APP;
  task.pUserTaskParam = ZW_system_startup_getAppHandles();
